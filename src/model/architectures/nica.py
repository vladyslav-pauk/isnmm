...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.trainable_parameter = nn.Parameter(torch.randn(1))  # Initialize with a random value

    def construct(self, latent_dim, observed_dim):
        self.loc_network = self.constructor(observed_dim, latent_dim, **self.config)
        self.scale_network = self.constructor(observed_dim, latent_dim, **self.config)
    ...
    def forward(self, x):
        ...
        # Append the trainable parameter to loc
        # trainable_param_expanded = self.trainable_parameter.expand(loc.size(0), -1)  # Match batch size
        # loc = torch.cat([loc, trainable_param_expanded], dim=1)  # Append along the feature dimension
