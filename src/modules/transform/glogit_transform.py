...
class GLogitTransform(Transform):
    ...
    def __call__(self, sample):
        # sample_padded = torch.cat([sample[..., :-1], sample[..., -1]], dim=-1)
        sample_padded = sample
        sample_padded[..., -1] = - 0.1 * torch.ones_like(sample_padded[..., -1])
    ...
    def _inverse(self, z):
        ...
        log_ratio = torch.cat([log_ratio, -0.1 * torch.ones_like(z[..., -1:])], dim=-1)
        return log_ratio

    def log_abs_det_jacobian(self, z, y):
        epsilon = 1e-12
        z = torch.clamp(z, min=epsilon)
        return -torch.sum(torch.log(z) - 0.1, dim=-1)
